  ...
  app.post(
    ...
    async (ctx: any) => {
      ...
      if (!selfie) {
        ctx.set.status = 400;
        return {
          error: ctx.t('user.selfieRequired'),
          message: 'A selfie image is required for age verification.',
        };
      }
      ...
      if (attempts >= 3) {
        ctx.set.status = 403;
        return {
          error: ctx.t('user.selfieAttemptsExceeded'),
          message: 'Maximum selfie verification attempts reached.',
        };
      }
      ...
      const dateOfBirth = body.dateOfBirth
        ...
        : user.dateOfBirth
          ? new Date(String(user.dateOfBirth))
          : null;
      if (!dateOfBirth || isNaN(dateOfBirth.getTime())) {
        ctx.set.status = 400;
        return {
          error: ctx.t('common.dateOfBirthRequired'),
          message: 'Your date of birth is required for selfie age verification.',
        };
      }
      ...
      try {
        ...
        if (predictedAge === null) {
          ctx.set.status = 400;
          return {
            error: ctx.t('common.noFaceDetected'),
            message: 'Could not detect a face in the selfie. Please try again.',
          };
        }

        const effectiveCountry =
          typeof body.billingCountry === 'string' ? body.billingCountry : user.billingCountry;
        ...
        if (difference > maxDelta) {
          ...
          return {
            error: 'age_mismatch',
            message: ctx.t('idVerification.ageMismatch', {
              predictedAge: predictedAge.toFixed(1),
              age,
            }),
            attempts: settings.ageVerificationSelfieAttempts,
            remaining,
    ...
    },
    {
      ...
      detail: {
        summary: 'Verify age from selfie',
        description:
          'Estimate a user age from selfie data and compare against the provided date of birth.',
        tags: ['Identity'],
      },
    }
  );
