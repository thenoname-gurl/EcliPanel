...
export async function rolloutRoutes(app: any, prefix = '') {
  ...
  app.get(
    prefix + '/admin/rollouts/:id/overrides',
    async (ctx: any) => {
      const id = Number(ctx.params?.id);
      if (!id || isNaN(id)) {
        ctx.set.status = 400;
  ...
  app.post(
    ...
    async (ctx: any) => {
      ...
      const treatment =
        body?.treatment !== undefined ? String(body.treatment).trim() || null : undefined;
      try {
  ...
  app.delete(
    prefix + '/admin/rollouts/:id/overrides/:userId',
    async (ctx: any) => {
      const id = Number(ctx.params?.id);
