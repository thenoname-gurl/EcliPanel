...
export async function oauthRoutes(app: any, prefix = '') {
  ...
  app.post(
    ...
    async ctx => {
      ...
      const validScopes = filterScopes(allowedScopes || ['profile', 'email'], [...OAUTH_SCOPES]);
  ...
  app.post(
    prefix + '/oauth/apps/:id/rotate-secret',
    async ctx => {
      const f = await requireFeature(ctx, 'oauth');
      if (f !== true) return f;
      const user = (ctx as any).user as User;
  ...
  app.post(
    ...
    async ctx => {
      ...
      if (!stateValid) {
        ctx.set.status = 400;
        return {
          error: ctx.t('common.invalidState'),
  ...
  app.post(
    ...
    },
    {
      ...
      response: {
        200: t.Any(),
        400: t.Object({ error: t.String(), error_description: t.Optional(t.String()) }),
        401: t.Object({ error: t.String(), error_description: t.Optional(t.String()) }),
      },
    }
  );

  app.post(
    prefix + '/oauth/token/revoke',
    async ctx => {
      const f = await requireFeature(ctx, 'oauth');
      if (f !== true) return f;
      ...
      const byAccess = await tokenRepo.findOne({
        where: { accessToken: token, app: { id: oauthApp.id } },
      });
      if (byAccess) {
      ...
      const byRefresh = await tokenRepo.findOne({
        where: { refreshToken: token, app: { id: oauthApp.id } },
      });
      if (byRefresh) {
  ...
  app.post(
    ...
    async ctx => {
      ...
      if (!tokenEntity || tokenEntity.revoked || new Date() > tokenEntity.accessTokenExpiresAt) {
        return { active: false };
      }
