...
function hostRepo() {
  return AppDataSource.getRepository(DatabaseHost);
}
function dbRepo() {
  return AppDataSource.getRepository(ServerDatabase);
}
function cfgRepo() {
  return AppDataSource.getRepository(ServerConfig);
}
...
export async function databaseRoutes(app: any, prefix = '') {
  ...
  app.post(
    ...
    async ctx => {
      ...
      try {
        ...
        ctx.set.status = 400;
        return {
          error: `Cannot connect to database server: ${sanitizeError(e, 'databaseHandler:test-connection')}`,
        };
      }
      const record = hostRepo().create({
        name,
        host,
        port,
        username,
  ...
  app.put(
    prefix + '/admin/database-hosts/:id',
    async ctx => {
      if (!requireDatabasePermission(ctx, 'write')) return;
  ...
  app.post(
    prefix + '/admin/database-hosts/:id/test',
    async ctx => {
      if (!requireDatabasePermission(ctx, 'write')) return;
      const { id } = ctx.params as any;
      const h = await hostRepo().findOneBy({ id: Number(id) });
  ...
  app.post(
    ...
    async ctx => {
      ...
      const user = ctx.user;
      const accountDatabaseLimit =
        user?.limits && typeof user.limits.databases === 'number' ? user.limits.databases : 0;
      if (accountDatabaseLimit > 0) {
        ...
        const existingAccountDbs =
          serverUuids.length > 0
            ? await dbRepo().count({ where: { serverUuid: In(serverUuids) } })
            : 0;
      ...
      try {
        ...
        ctx.set.status = 500;
        return {
          error: `Failed to create database: ${sanitizeError(e, 'databaseHandler:create-db')}`,
        };
      }
      await conn.end();
  ...
  app.delete(
    prefix + '/servers/:id/databases/:dbId',
    async ctx => {
  ...
  app.get(
    prefix + '/servers/:id/databases/:dbId/credentials',
    async ctx => {
      const { id, dbId } = ctx.params as any;
      const db = await dbRepo().findOneBy({ id: Number(dbId), serverUuid: id });
      if (!db) {
...
function requireDatabasePermission(ctx: any, level: 'read' | 'write'): boolean {
  ...
  if (!user) {
    ctx.set.status = 401;
    (ctx as any).body = { error: ctx.t('auth.unauthorized') };
    return false;
  }
  const permission = level === 'write' ? 'databases:write' : 'databases:read';
  if (!hasPermissionSync(ctx, permission)) {
    ctx.set.status = 403;
    (ctx as any).body = { error: ctx.t('validation.databaseAccessRequired') };
    return false;
  }
  return true;
}
