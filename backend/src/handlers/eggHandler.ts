  ...
  app.post(
    ...
    async ctx => {
      ...
      if (raw.config?.files) {
        try {
          configFiles =
            typeof raw.config.files === 'string' ? JSON.parse(raw.config.files) : raw.config.files;
        } catch {
      ...
      processConfig = {
        ...
        stop: {
          type: stopValue === 'SIGKILL' ? 'kill' : stopValue === 'SIGTERM' ? 'stop' : 'command',
          value: stopValue,
        },
      ...
      const fileDenylist: string[] | undefined = Array.isArray(raw.file_denylist)
        ? raw.file_denylist
        : undefined;
