...
export async function planRoutes(app: any, prefix = '') {
  ...
  app.post(
    ...
    },
    {
      ...
      response: {
        200: t.Object({ success: t.Boolean(), plan: t.Any() }),
  ...
  app.put(
    ...
    async ctx => {
      ...
      if (serverLimit !== undefined)
        plan.serverLimit = serverLimit != null ? Number(serverLimit) : undefined;
      if (databases !== undefined)
        plan.databases = databases != null ? Number(databases) : undefined;
      if (backups !== undefined) plan.backups = backups != null ? Number(backups) : undefined;
      if (emailSendDailyLimit !== undefined)
        plan.emailSendDailyLimit =
          emailSendDailyLimit != null ? Number(emailSendDailyLimit) : undefined;
      if (emailSendQueueLimit !== undefined)
        plan.emailSendQueueLimit =
          emailSendQueueLimit != null ? Number(emailSendQueueLimit) : undefined;
  ...
  app.post(
    prefix + '/admin/plans/:id/reapply-limits',
    async ctx => {
      const plan = await planRepo().findOneBy({ id: Number((ctx.params as any).id) });
      if (!plan) {
        ctx.set.status = 404;
      ...
      if (plan.emailSendDailyLimit != null)
    ...
    },
    {
      ...
      detail: {
        summary: 'Reapply plan limits to users without custom limits',
        tags: ['Plans', 'Admin'],
      },
      response: {
