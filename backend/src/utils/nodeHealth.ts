...
export async function getLastNodeHeartbeats(): Promise<
  Map<number, { status: string; timestamp: Date }>
> {
  const repo = AppDataSource.getRepository(NodeHeartbeat);
  const table = repo.metadata.tableName;
  const rows: Array<{ nodeId: number; status: string; timestamp: Date | string }> =
    await repo.query(
