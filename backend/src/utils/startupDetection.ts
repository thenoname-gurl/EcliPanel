...
export function normalizeStartupDonePatterns(input: unknown): string[] {
  ...
  const patterns = raw.map(value => String(value ?? '')).filter(value => value.length > 0);

  if (patterns.length === 0) return [DEFAULT_STARTUP_DETECTION_PATTERN];
  return patterns;
}

export function normalizeProcessConfig(
  processConfig: Record<string, any> | null | undefined
): Record<string, any> | null {
  if (processConfig === null || processConfig === undefined) return processConfig ?? null;
  if (typeof processConfig !== 'object' || Array.isArray(processConfig))
    return processConfig as any;
