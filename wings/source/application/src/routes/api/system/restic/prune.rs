...
static RESTIC_PRUNE_CACHE: RwLock<Option<ResticTaskResult>> = RwLock::const_new(None);
...
mod get {
    ...
    pub async fn route() -> ApiResponseResult {
        ...
        match &*cache {
            ...
            None => ApiResponse::error("no restic prune result available yet")
...
mod post {
    ...
    pub async fn route(
        ...
        {
            let mut running = super::RESTIC_PRUNE_RUNNING.write().await;
            if *running {
                return ApiResponse::error("restic prune task is already running")
        ...
        if data.foreground {
            let command = build();
            let result = execute_restic_command(command, "prune").await;

            *super::RESTIC_PRUNE_CACHE.write().await = Some(result.clone());
            *super::RESTIC_PRUNE_RUNNING.write().await = false;
            ...
            tokio::spawn(async move {
                ...
                *super::RESTIC_PRUNE_CACHE.write().await = Some(result);
                *super::RESTIC_PRUNE_RUNNING.write().await = false;
            });
