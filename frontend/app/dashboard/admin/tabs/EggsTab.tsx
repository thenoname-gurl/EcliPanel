  ...
  return (
      ...
      {eggs.length === 0 ? (
        ...
        <>
                  ...
                  {eggs.map((egg: any, i: number) => (
                    <tr key={egg.id ?? i} className="border-b border-border/50 last:border-0 hover:bg-secondary/20 group">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="h-8 w-8 bg-secondary/80 flex items-center justify-center shrink-0">
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center bg-secondary/50 px-2 py-0.5 font-mono text-xs text-muted-foreground max-w-[200px] truncate">
                          {egg.dockerImage}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-0.5 opacity-60 group-hover:opacity-100 transition-opacity">
                          ...
                          <button
                            ...
                            className="p-1.5 text-muted-foreground hover:bg-primary/10 hover:text-primary transition-colors disabled:opacity-40"
                          >
          ...
          <div className="flex flex-col gap-3 md:hidden">
            {eggs.map((egg: any, i: number) => (
              <div key={egg.id ?? i} className="border border-border bg-card overflow-hidden">
                <div className="flex items-start gap-3 p-4 pb-3">
                  <div className="h-10 w-10 bg-secondary/80 flex items-center justify-center shrink-0">
    ...
    <Dialog open={importEggOpen} onOpenChange={(open) => { if (!open) { setImportEggOpen(false); setImportEggPreview(null); setImportEggError("") } }}>
      <DialogContent className="border-border bg-card sm:max-w-xl">
        ...
        {importEggPreview ? (
          ...
          <div className="flex flex-col gap-4 py-2">
            <div className="flex gap-1 border border-border p-1 bg-secondary/20">
              <button
                onClick={() => setImportEggMode("paste")}
                className={`flex-1 px-3 py-1.5 text-xs font-medium transition-colors ${importEggMode === "paste" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
              >
                {t("importDialog.modes.pasteJson")}
              </button>
              <button
                onClick={() => setImportEggMode("url")}
                className={`flex-1 px-3 py-1.5 text-xs font-medium transition-colors ${importEggMode === "url" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
              >
            ...
            {importEggMode === "paste" ? (
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  className="h-52 w-full border border-border bg-secondary/30 p-3 font-mono text-xs text-foreground placeholder:text-muted-foreground resize-none focus:outline-none focus:ring-1 focus:ring-primary"
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  className="w-full border border-border bg-secondary/30 px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary"
            ...
            <DialogFooter>
              ...
              <Button onClick={doImportEgg} disabled={importEggLoading} className="bg-primary text-primary-foreground">
                {importEggLoading ? <><Loader2 className="h-3.5 w-3.5 rounded-full animate-spin mr-1.5" /> {t("actions.importing")}</> : t("actions.importEgg")}
              </Button>
            </DialogFooter>
          </div>
        )}
    ...
    <Dialog open={eggDialog !== null} onOpenChange={(open) => !open && setEggDialog(null)}>
      <DialogContent className="border-border bg-card sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex gap-1 border border-border p-1 bg-secondary/20 mt-1">
          {(["basic", "variables", "config", "advanced"] as const).map((tabKey) => (
            <button key={tabKey} onClick={() => setEggTab(tabKey)}
              className={`flex-1 px-2 py-1.5 text-xs font-medium capitalize transition-colors ${eggTab === tabKey ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}>
        ...
        <div className="flex flex-col gap-3 py-1">
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input value={eggName} onChange={(e) => setEggName(e.target.value)} className="border border-border bg-secondary/50 px-3 py-2 text-sm text-foreground outline-none focus:border-primary/50" placeholder="Minecraft Java" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("fields.author")}</label>
                  <input value={eggAuthor} onChange={(e) => setEggAuthor(e.target.value)} className="border border-border bg-secondary/50 px-3 py-2 text-sm text-foreground outline-none focus:border-primary/50" placeholder="support@pterodactyl.io" />
                </div>
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("fields.description")}</label>
                <input value={eggDesc} onChange={(e) => setEggDesc(e.target.value)} className="border border-border bg-secondary/50 px-3 py-2 text-sm text-foreground outline-none focus:border-primary/50" placeholder="Optional description" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("fields.primaryDockerImage")}</label>
                <input value={eggImage} onChange={(e) => setEggImage(e.target.value)} className="border border-border bg-secondary/50 px-3 py-2 text-sm font-mono text-foreground outline-none focus:border-primary/50" placeholder="ghcr.io/pterodactyl/yolks:java_21" />
              </div>
              <div className="flex flex-col gap-1.5">
                ...
                <textarea value={eggDockerImagesRaw} onChange={(e) => setEggDockerImagesRaw(e.target.value)} rows={3} className="border border-border bg-secondary/50 px-3 py-2 text-xs font-mono text-foreground outline-none focus:border-primary/50 resize-none" placeholder={'{\n  "Java 21": "ghcr.io/pterodactyl/yolks:java_21",\n  "Java 17": "ghcr.io/pterodactyl/yolks:java_17"\n}'} />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("fields.startupCommand")}</label>
                <input value={eggStartup} onChange={(e) => setEggStartup(e.target.value)} className="border border-border bg-secondary/50 px-3 py-2 text-sm font-mono text-foreground outline-none focus:border-primary/50" placeholder="java -Xms128M -Xmx{{SERVER_MEMORY}}M -jar {{SERVER_JARFILE}}" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input value={eggFeatures} onChange={(e) => setEggFeatures(e.target.value)} className="border border-border bg-secondary/50 px-3 py-2 text-sm text-foreground outline-none focus:border-primary/50" placeholder="eula, java_version, pid_limit" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Update URL</label>
                  <input value={eggUpdateUrl} onChange={(e) => setEggUpdateUrl(e.target.value)} className="border border-border bg-secondary/50 px-3 py-2 text-sm text-foreground outline-none focus:border-primary/50" placeholder="https://…" />
                </div>
              </div>
              <div className="flex flex-col gap-1.5">
                ...
                <textarea value={eggFileDenylist} onChange={(e) => setEggFileDenylist(e.target.value)} rows={2} className="border border-border bg-secondary/50 px-3 py-2 text-sm font-mono text-foreground outline-none focus:border-primary/50 resize-none" placeholder="/.env" />
              </div>
          ...
          {eggTab === "variables" && (
            <div className="flex flex-col gap-2">
              ...
              <textarea value={eggEnvVars} onChange={(e) => setEggEnvVars(e.target.value)} rows={6} className="border border-border bg-secondary/50 px-3 py-2 text-sm font-mono text-foreground outline-none focus:border-primary/50 resize-none" placeholder={"SERVER_MEMORY\nSERVER_JARFILE\nMC_VERSION"} />
              {(eggDialog !== "new" && eggDialog) && (
                <div className="border border-border bg-secondary/20 p-3 flex flex-col gap-2 text-xs text-muted-foreground">
                  ...
                  <div className="flex flex-col gap-2">
                    {(eggEnvVarDefs || []).map((v: any, i: number) => (
                      <div key={i} className="grid grid-cols-12 gap-2 items-center">
                        ...
                        }} className="col-span-3 border border-border bg-secondary/50 px-2 py-1 text-sm font-mono" />
                        <input value={v.default_value ?? v.defaultValue ?? ""} onChange={(e) => {
                          const next = [...(eggEnvVarDefs || [])]
                          next[i] = { ...next[i], default_value: e.target.value }
                          setEggEnvVarDefs(next)
                        }} className="col-span-4 border border-border bg-secondary/50 px-2 py-1 text-sm font-mono" placeholder="default value" />
                        <input value={v.description ?? ""} onChange={(e) => {
                          const next = [...(eggEnvVarDefs || [])]
                          next[i] = { ...next[i], description: e.target.value }
                          setEggEnvVarDefs(next)
                        }} className="col-span-4 border border-border bg-secondary/50 px-2 py-1 text-sm" placeholder="description" />
          ...
          {eggTab === "config" && (
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-1.5">
                ...
                <input value={eggProcessStop} onChange={(e) => setEggProcessStop(e.target.value)} className="border border-border bg-secondary/50 px-3 py-2 text-sm font-mono text-foreground outline-none focus:border-primary/50" placeholder="stop" />
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea value={eggProcessDone} onChange={(e) => setEggProcessDone(e.target.value)} rows={6} className="border border-border bg-secondary/50 px-3 py-2 text-sm font-mono text-foreground outline-none focus:border-primary/50 resize-none" placeholder={"Done ("} />
            ...
            <div className="flex flex-col gap-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input value={eggInstallContainer} onChange={(e) => setEggInstallContainer(e.target.value)} className="border border-border bg-secondary/50 px-3 py-2 text-sm font-mono text-foreground outline-none focus:border-primary/50" placeholder="ghcr.io/pterodactyl/installers:debian" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("fields.entrypoint")}</label>
                  <input value={eggInstallEntrypoint} onChange={(e) => setEggInstallEntrypoint(e.target.value)} className="border border-border bg-secondary/50 px-3 py-2 text-sm font-mono text-foreground outline-none focus:border-primary/50" placeholder="bash" />
                </div>
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">{t("fields.installScript")}</label>
                <textarea value={eggInstallScript} onChange={(e) => setEggInstallScript(e.target.value)} rows={14} className="border border-border bg-secondary/50 px-3 py-2 text-xs font-mono text-foreground outline-none focus:border-primary/50 resize-none" placeholder={"#!/bin/bash\napt-get install -y curl\n# ...\n"} />
              </div>
              <div className="flex flex-col gap-1.5">
