    ...
    <RolloutGuard rolloutKey="elo_rating" fallback={
      <div className="flex items-center justify-center h-[60vh]">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-lg font-semibold text-foreground mb-2">{t("leaderboard.comingSoon")}</h2>
          <p className="text-sm text-muted-foreground">
            {t("leaderboard.rolloutMessage")}
          </p>
        </div>
      </div>
    }>
          ...
          {loading ? (
            ...
            <section className="border border-border/50 bg-card shadow-sm p-6 sm:p-8 text-center">
              <div className="flex flex-col items-center justify-center py-16 text-center px-6 gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-foreground">{t("leaderboard.noRankedProjects")}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {t("leaderboard.startVoting")}
                  </p>
                </div>
                <Link
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border text-xs text-muted-foreground">
                      <th className="px-5 py-3 text-left font-medium">{t("leaderboard.table.rank")}</th>
                      <th className="px-5 py-3 text-left font-medium">{t("leaderboard.table.project")}</th>
                      <th className="px-5 py-3 text-right font-medium">{t("leaderboard.table.elo")}</th>
                      <th className="px-5 py-3 text-right font-medium">{t("leaderboard.table.votes")}</th>
                      <th className="px-5 py-3 text-right font-medium">{t("leaderboard.table.wl")}</th>
                      <th className="px-5 py-3 text-right font-medium">{t("leaderboard.table.winRate")}</th>
                      <th className="px-5 py-3 text-right font-medium">{t("leaderboard.table.owner")}</th>
                    </tr>
                  </thead>
                  <tbody>
