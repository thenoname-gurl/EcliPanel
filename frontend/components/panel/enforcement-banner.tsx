...
export function EnforcementBanner() {
  ...
  if (emailDone && passkeyDone) return null
      ...
      <div className="flex flex-col gap-3 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-foreground">Account requirements incomplete</p>
            <p className="text-xs text-muted-foreground">
              You must verify your email and register a passkey to use this panel.
            </p>
          </div>
        </div>
        ...
        <div className="ml-8 flex flex-col gap-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`text-sm ${emailDone ? "text-success" : "text-foreground"}`}>
              {emailDone ? "Email verified" : "Email not verified"}
            </span>
            {!emailDone && !showCodeInput && (
              ...
              >
                {resending ? "Sending…" : resent ? "Re-send" : "Send verification email"}
              </button>
            )}
          </div>
          ...
          {/* Passkey */}
          {(() => {
            const isSecure = typeof window !== "undefined" && window.isSecureContext && !!navigator.credentials;
            return (
              <div className="flex items-center gap-3">
                {passkeyDone ? (
                  <CheckCircle className="h-4 w-4 text-success" />
                ) : (
                  <KeyRound className={`h-4 w-4 ${isSecure ? "text-warning" : "text-muted-foreground"}`} />
                )}
                <span className={`text-sm ${passkeyDone ? "text-success" : "text-foreground"}`}>
                  {passkeyDone ? "Passkey registered" : "No passkey registered"}
                </span>
                {!passkeyDone && (
                  isSecure ? (
                    <a
                    ...
                    <span className="ml-2 text-xs text-muted-foreground">
                      Requires HTTPS — access the panel via <code className="font-mono">https://</code> to register a passkey.
                    </span>
                  )
                )}
              </div>
